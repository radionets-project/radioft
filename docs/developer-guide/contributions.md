...
the maintainers can provide you access to the main repository at https://github.com/radionets-project/radioft.
...
If you are an external contributor, you will have to create a fork of `radioft`.
...
```shell-session
$ git clone git@github.com:radionets-project/radioft.git
```
...
In order for you to contribute to `radioft` without write access in the main repository,
you will need to [fork the repository][github-fork] first. After you have created the fork, clone
it:
```shell-session
$ git clone https://github.com/[YOUR-GITHUB-USERNAME]/radioft.git
```
...
```shell-session
$ git remote add upstream https://github.com/radionets-project/radioft.git
```
```{seealso}
...
of the main repository https://github.com/radionets-project/radioft.
...
We're using [`pytest`][pytest] to test the functionality of `radioft`. To test all
components, run:
```shell-session
$ pytest -vv
```
