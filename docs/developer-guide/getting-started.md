...
`radioft` package, if you just want to write code that uses it
as a dependency, you can install `radioft` through one of the
...
We provide a [`mamba`][mamba]/`conda` environment with all packages needed for development of `radioft`
...
## Installing `radioft` in Development Mode

:::{note}
We recommend using the `uv` package manager to install ``radioft``
and its dependencies. Never heard of `uv`? See [the documentation][uv] for more.
:::

To install `radioft` in your virtual environment, just run
...
provides all the necessary dependencies for development on `radioft`.
:::
...
```
The [Ruff][ruff] hook uses the configuration in [`pyproject.toml`][radioft-pyproject] for linting and formatting.


## Next Steps

Check out {ref}`contributions` and {ref}`coding-style` to learn how to contribute
to `radioft` as a developer.
...
[radioft-pyproject]: https://github.com/radionets-project/radioft/blob/main/pyproject.toml
